...
export function buildGenerateTripPrompt(input: TripPlanningInput): TaskPrompt {
  return {
    systemInstruction: [
      ...
      'When verified candidates are supplied, prefer them and copy their provider and providerPlaceId exactly. Never mark any other place as verified.',
    ...
    userData: wrapUntrustedUserData({ request: input.prompt, intent: input.intent, verifiedCandidates: input.candidates }),
  };
}
