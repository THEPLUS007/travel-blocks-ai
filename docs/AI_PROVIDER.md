...
현재 실행 provider는 **Gemini 하나뿐**이며 API composition root는 provider-neutral `AiTaskRouter`에 Gemini registration을 주입합니다. 정적 routing table은 네 logical task를 모두 Gemini로 결정하고, construction 시 `AI_TASK_DEFINITIONS`의 required capability를 등록 capability와 검증합니다. `GEMINI_INTENT_MODEL`을 통한 task별 모델 선택은 provider routing이 아닙니다. self-hosted LLM adapter foundation은 구현되었지만 dynamic routing·fallback routing·AI scope contract는 아직 구현되지 않았습니다. Google Places 같은 factual data provider와 LLM provider는 별도 boundary이고, grounding 및 deterministic feasibility는 LLM adapter 밖의 application/domain 책임입니다. P1-5 target과 단계별 실행 조건은 [Roadmap](ROADMAP.md), 현재 상태는 [Current Status](CURRENT_STATUS.md), 변경 불가 경계는 [Invariants](INVARIANTS.md)를 따릅니다.
...
`planTrip(TripPlanningInput)` is the provider operation for the logical `generate_trip` task. `generateTrip(GenerateTripInput)` remains the existing compatibility/convenience method and preserves the public API flow. Ranking candidate semantic refinement, analyze-text source-only normalization, structured-output validation, and observability task names remain unchanged. `AiTaskRouter` selects one registered provider and delegates once without prompt construction, parsing, retry, timeout, dedupe, concurrency, grounding, fallback, or observability work. Gemini remains the only execution provider; P1-5C self-hosted provider, dynamic routing, and fallback execution are not implemented.

## P1-5C self-hosted adapter foundation

`SelfHostedTravelAiProvider` is an injectable internal transport adapter. P1-5C supports only `extract_intent`, validates the existing task input/output schemas, normalizes timeout/network/HTTP/auth/invalid-output failures to `AiProviderError`, and records only safe lifecycle metadata. `SELF_HOSTED_LLM_ENABLED` is false by default; when enabled, endpoint, model, optional token, and timeout are configuration-only inputs. The internal HTTP transport posts `{ task, model, input }` to `/v1/generate` and expects `{ content: string }`. Gemini remains the active provider for every production task; there is no fallback, dynamic routing, live self-hosted call, or deployment. P1-5D owns eligibility, health, and routing policy.
