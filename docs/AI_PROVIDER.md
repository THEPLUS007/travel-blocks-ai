...
현재 실행 provider는 **Gemini 하나뿐**입니다. `GEMINI_INTENT_MODEL`을 통한 task별 모델 선택은 multi-provider router가 아니며, self-hosted LLM provider·fallback routing·AI scope contract는 아직 구현되지 않았습니다. Google Places 같은 factual data provider와 LLM provider는 별도 boundary이고, grounding 및 deterministic feasibility는 LLM adapter 밖의 application/domain 책임입니다. P1-5 target과 단계별 실행 조건은 [Roadmap](ROADMAP.md), 현재 상태는 [Current Status](CURRENT_STATUS.md), 변경 불가 경계는 [Invariants](INVARIANTS.md)를 따릅니다.
