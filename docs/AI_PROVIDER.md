...
Terminal `AiProvenance` contains `executionId`, version, task, capability, provider ID, model, routing mode/reason, start/completion timestamps, duration, outcome, and failure category when the existing error is an `AiProviderError`; unclassified errors are `unknown`. It contains no prompt, input, output, raw provider response, token, authorization header, endpoint, stack trace, or traveler content. It is delivered to an observer only; observer failure is reported through the existing safe callback and never changes result/error identity or causes another provider call. P1-5E does not persist provenance, expose it through public API, alter factual Places/opening-hours provenance, change routes, or add fallback/retry. P1-5F remains benchmark/quality-gate work.


## P1-5F local bake-off (2026-09-23)

P1-5F executed an isolated ARM CPU bake-off; no local candidate is eligible for staging. Candidate A was excluded before download because its community GGUF did not disclose the exact conversion-version provenance required for reproduction. Official Qwen3-4B Q4_K_M and Gemma 4 E2B Q4_0 loaded but each first synthetic `extract_intent` warm-up exceeded the 30-second hard budget and was stopped; no fallback occurred. The provider/task matrix remains Gemini for all four tasks. See `P1-5F_LOCAL_MODEL_BAKEOFF.md` for provenance and safe aggregate measurements.
