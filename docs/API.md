...
Gemini가 schema에 맞지 않는 structured output을 반환하면 `AI_INVALID_OUTPUT`/502, provider 자체가 unavailable하면 `AI_PROVIDER_UNAVAILABLE`/503으로 구분합니다. 두 경우 모두 raw provider 응답이나 내부 stack은 노출하지 않습니다.
