...
it('maps an AI lifecycle event without prompt, response, key, or fake trip ID', () => {
  const event: AiRunEvent = { provider: 'gemini', model: 'planning', task: 'extract_intent', status: 'success', latencyMs: 42, inputTokens: 5, outputTokens: 3, providerAttempts: 1, retryAfterUsed: false };
...
it('maps a known failure code', () => {
  const event: AiRunEvent = { provider: 'gemini', model: 'planning', task: 'rank_places', status: 'error', latencyMs: 10, errorCode: 'timeout', providerAttempts: 1, retryAfterUsed: false };
